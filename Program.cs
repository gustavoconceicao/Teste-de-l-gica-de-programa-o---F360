...
namespace Teste_de_Klingon
{
    class Program
    {
        ...
        string[] palavras = textoB.Split(' ');
        int preposicoes = 0;
        int verbos = 0;
        int verbosFirstPerson = 0;
        int numerosBonitos = 0;

            for (int i = 0; i<= palavras.Length-1; i++)
            {
                if (checkPrep(palavras[i]))
                {
                    preposicoes++;                    
                }

                if (checkVerbo(palavras[i]))
                {
                    verbos++;
                }
                if (checkVerboFirstPerson(palavras[i]) && checkVerbo(palavras[i])){
                    verbosFirstPerson++;
                }

                if (numeroBonito(palavras[i]) >= 440566 && numeroBonito(palavras[i]) % 3 == 0)
                {
            ...
            String[] palavrasSemRepetir = palavras.Distinct().ToArray();
            Console.WriteLine("TAMANHO: " + palavras.Length + "TAMANHO SEM REPETIR: " + palavrasSemRepetir.Length);

            Array.Sort(palavrasSemRepetir, new CustomSort());
            foreach (var item in palavrasSemRepetir) { Console.Write(item + " "); }

            Console.WriteLine(preposicoes + " Preposiçoes encontradas");
            Console.WriteLine(verbos + " Verbos encontradas");
            Console.WriteLine(verbosFirstPerson + " Verbos em primeira pessoa");
            Console.WriteLine(numerosBonitos + " Numeros bonitos");
            Console.ReadLine();
        
        }

        public static bool checkPrep(string palavra)
        {
           if (!checkD (palavra) && !checkBar(palavra) && checkTam(palavra, 3, "menor"))
            {
                return true;
            }
        ...
        public static bool checkVerbo ( string palavra)
        {
            if (checkTam (palavra, 8, "maior") && checkBar(palavra))
            {
                return true;
            }
        ...
        public static bool checkVerboFirstPerson ( string palavra)
        {
        ...
        public static bool checkD(string palavra)
        {
            bool x;
            return x =  palavra.Contains("d");
        }

        public static bool checkBar (string palavra)
        {
            if (palavra.EndsWith("s") || palavra.EndsWith("l") || palavra.EndsWith("f") || palavra.EndsWith("w") || palavra.EndsWith("k"))
            {
                return true;
            }
            return false;
        ...
        public static bool checkTam(string palavra, int tam, string operacao)
        {
        ...
        public static double numeroBonito (string palavra)
        {
            var sortOrder = new List<char> { 'k', 'b', 'w', 'r', 'q', 'd', 'n', 'f', 'x', 'j', 'm', 'l', 'v', 'h', 't', 'c', 'g', 'z', 'p', 's' };
            double soma = 0;


            for (int i =0; i<palavra.Length; i++)
            {               
                soma += (sortOrder.IndexOf(palavra[i]) * Math.Pow(20, i));
            }
    ...
    class CustomSort : IComparer<String>
    {
        ...
        //https://stackoverflow.com/questions/29245800/custom-char-order
    }
}
